...
export async function getOrders(first = 20) {
  return shopifyGraphql<{ orders: { edges: { node: any }[] } }>(`query Orders($first: Int!) { orders(first: $first, sortKey: CREATED_AT, reverse: true) { edges { node { id name createdAt displayFinancialStatus displayFulfillmentStatus totalPriceSet { shopMoney { amount currencyCode } } shippingAddress { name address1 address2 city province country countryCode zip phone } lineItems(first: 50) { nodes { id title quantity sku variant { id } product { id title } } } } } }`, { first });
}
